...
export class ESPWebToolsManager {
  ...
  async connectDevice(): Promise<DeviceInfo> {
    try {
      ...
      // Try different configurations for ESP32 devices
      const configurations: SerialOptions[] = [
        { baudRate: 115200, dataBits: 8, stopBits: 1, parity: 'none' as const, bufferSize: 255, flowControl: 'none' as const },
        { baudRate: 115200, dataBits: 8, stopBits: 1, parity: 'none' as const, bufferSize: 1024, flowControl: 'hardware' as const },
        { baudRate: 460800, dataBits: 8, stopBits: 1, parity: 'none' as const, bufferSize: 255, flowControl: 'none' as const }
      ];
      ...
      for (const config of configurations) {
        try {
          console.log('Trying configuration:', config);
          await this.port.open(config);
          console.log('Port opened successfully with config:', config);
          portOpened = true;
          break;
        } catch (error) {
      ...
      if (!portOpened) {
        throw new Error(`Failed to open serial port with any configuration. Last error: ${lastError?.message || 'Unknown error'}`);
      }
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Access denied')) {
          throw new Error('Device access denied. Please close any other applications using this device (Arduino IDE, PlatformIO, etc.) and try again.');
        }
        if (error.message.includes('Failed to open serial port')) {
          throw new Error('Failed to open device. Ensure your ESP32 is connected via USB, drivers are installed, and no other software is using the device.');
        }
        if (error.message.includes('Device not found')) {
          throw new Error('ESP32 device not found. Please check your USB connection and ensure the device is powered on.');
        }
      }
